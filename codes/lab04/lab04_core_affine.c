...
#include<sys/time.h>
#include<pthread.h> 
#include<sched.h> // Include this header for CPU affinity
...
void master(int matrixSize) {
    ...
    int core_counter = 0; // To assign threads to cores in a round-robin manner
    ...
    for (i = 0; i < t; i++) {
        ...
        *arrayOfSlaves[i].new_sock = socket_desc;
        ...
        set_affinity(core_counter % num_cores);
        core_counter++;
